...
@Data
public class IntegerComputerV5 {
    private int count;
    private int oppCode;
    ...
    private List<Integer> inputProgram ;
    ...
    public IntegerComputerV5(List<Integer> inputProgram) {
        this.count = 0;
        this.oppCode = 0;
        ...
        for (int x = inputProgram.size(); x < 100000; x++) inputProgram.add((0));
    }

    private int getOppCode(int x) {
        return ((((inputProgram.get(x) / 10) % 10) * 10) + (inputProgram.get(x) % 10));
    }

    private int getParameter(int parameterNumber) {
    ...
    private int getPosition(int parameterNumber) {
        ...
        int mode = ((instruction / (10 * (int) Math.pow(10, parameterNumber))) % 10);
        switch (mode) {
            case 0 : {
    ...
    public List<Integer> runIntegerComputer() {
        hasOutput = false;
        wantsInput = false;

        while ((!halted) && (!hasOutput) && (!wantsInput))  {
